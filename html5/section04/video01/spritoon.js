...
class Sprite {
	...
	set index(value){
		...
		if (this.game==undefined && !this.frameData.isArray()){
		...
		const formatPos = { start:this._frame.indexOf("{") };
	...
	setRest(restore=true){
		...
		if (restore){
			...
			this.rest = {};
			if (this.frame.index == null && this.frames!=null){
				let index = 0;
				for(let frame of this.frames){
					frame.index = index;
					index++;
			...
			this.rest.imageIndex = this.frames.index;
