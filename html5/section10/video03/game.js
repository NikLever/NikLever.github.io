...
class Game{
	...
	computerMove(){
		...
		for(let sprite of this.sprites){
			sprite.update(0);
			tiles.push(new Tile(sprite.row, sprite.col, sprite.frameData.filename=="tile_001.png"));
		}
		let boards = [];
		for(let row=0; row<8; row++){
			for(let col=0; col<8; col++){
				if (this.legalMove(row, col)){
					...
					const tile = new Tile(row, col, true);
		...
		if (boards.length>0){
			...
			for(let board of boards){
				const bscore = board.score.black;
