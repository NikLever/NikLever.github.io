	...
	**/
	set anim(value){
		...
		if (value.motion!==undefined){
            if (value.motion.x !== undefined && typeof(value.motion.x)==='object'){
               let options = value.motion.x.easing;
			     //(start, end, duration, startTime=0, type='linear')
			     this._anim.easingX = new Easing(value.y, value.y + options.change, options.duration, 0, options.type); 
            }
			if (value.motion.y !== undefined && typeof(value.motion.y)==='object'){
               let options = value.motion.y.easing;
			     //(start, end, duration, startTime=0, type='linear')
			     this._anim.easingY = new Easing(value.y, value.y + options.change, options.duration, 0, options.type); 
            }
		}
		this.animTime = 0;
	}
	...
	update(dt){
		if (this._anim!=null){
			...
			if (this.physicsBody==null && this._anim.motion!=null){
				if (this._anim.motion.x !== undefined){
					if (typeof(this._anim.motion.x)==='object'){
						this.x = this._anim.easingX.value(this.animTime);
					}else{
						this.x += ((this.flipped) ? -this._anim.motion.x : this._anim.motion.x) * dt;
					}
				}
				if (this._anim.motion.x !== undefined){
					if (typeof(this._anim.motion.y)==='object'){
						this.y = this._anim.easingY.value(this.animTime);
					}else{
...
class Rect{
	...
	/**
	@returns {number} the position of the left edge
	**/
	get left(){
		return this.x;
	}
	...
	/**
	@param {number} sets the position of the left edge
	**/
	set left(value){
		this.x = value;
	}
	...
	/**
	@returns {number} the position of the right edge
	**/
	get right(){
	...
	/**
	@param {number} sets the position of the right edge
	**/
	set right(value){
		this.w = value - this.x;
	}
	...
	/**
	@returns {number} the position of the top edge
	**/
	get top(){
		return this.y;
	}
	...
	/**
	@param {number} sets the position of the top edge
	**/
	set top(value){
		this.y = value;
	}
	...
	/**
	@returns {number} the position of the bottom edge
	**/
	get bottom(){
	...
	/**
	@param {number} sets the position of the bottom edge
	**/
	set bottom(value){
		this.h = value - this.y;
	}
	...
	**/
	...
	/**
	@param {number} sets the width of the rectangle
	**/
	set width(value){
		this.w = value;
	}
	...
	**/
	...
	/**
	@param {number} sets the height of the rectangle
	**/
	set height(value){
		this.h = value;
	}
	...
	**/
	...
	/**
	@param {Rect} r - another Rect
	@returns {boolean} true of Rect r overlaps with this Rect
	**/
	overlaps(r){
...
@param {object} options - an options object to handle the button<br>
{string} text - the text on this button - defaults to no text
{string} font - the font to use - defaults to Verdana
{Rect} rect - A Rect that gives the position and size of the Button in canvas coordinates
{string} color - the color of the text in css style eg #ffffff would be white
{string} background - the color of the background in css style eg #ffffff would be white
{number} fontSize - the size of the font in pixels
{function} onClick - the code to run when a click occurs
**/
class Button{
	constructor(options={}){
	...
	/**
	@param {object} mousePos - the x,y position of a click
	@returns {boolean} true if in the rectangle of this button
	**/
	hitTest(mousePos){
	...
	/**
	Paints the button to the context
	@param {object} context - a canvas 2d context
	**/
	render(context){
...
@param {object} options - an options object to handle initialising the class<br>
{object} context - an AudioContext object<br>
{number} volume - a number 0-1 to control the volume - default 1<br>
{boolean} loop - does this sfx loop - default false<br>
{number} fadeDuration - a number that controls the change in volume speed - default 0.5<br>
...
class SFX{
	...
	/**
	Does the browser support the sound type
	@param {string} type - eg. mp3 or webm
	**/
