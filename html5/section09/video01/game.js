class Game{
	...
	init(){
		...
		this.anims.push(new Anim("jump", {frameData:this.spriteData.frames, frames:[41,"..",58], motion:{x:100, y:{easing:{type:"outQuad", change:-150, duration:18/30}}}, loop:false, oncomplete:function(){
			game.setAction("xbloke", "drop");
		}}));
		this.anims.push(new Anim("drop", {frameData:this.spriteData.frames, frames:[59,"..",73], motion:{x:150, y:{easing:{type:"inQuad", change:150, duration:15/30}}}, loop:false, oncomplete:function(){
			game.checkFloor();
		}}));
		...
		this.anims.push(new Anim("kick", {frameData:this.spriteData.frames, frames:[580,"..",605], motion:{x:50, y:{easing:{type:"projectile", change:-30, duration:25/30}}}, loop:false, oncomplete:function(){
			game.checkFloor();
			game.setAction("xbloke", "ambient");
		}}));
	...
	checkFloor(){
		...
		if (this.xbloke.onground){
			if (posY<floor){
				...
				for(let platform of this.bitsData.platforms){
					let frame;
					for(let frameData of this.bitsData.frames){
						if (frameData.filename==platform.filename){
